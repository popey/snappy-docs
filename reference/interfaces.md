...
| `desktop` | Can access basic desktop graphical resources (_2.28+_). This interface is designed to be used with other interfaces, such as wayland. | yes | no |  |
| `desktop-legacy` | Can access desktop legacy methods such as a11y and input methods which gives privileged access to sensitive information and does not prevent eavesdropping or apps interfering with each other (_2.28+_). | yes | yes |  |
